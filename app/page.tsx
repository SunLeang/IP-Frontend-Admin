  ...
  function DropdownMenu() {
    ...
    return (
      <div className="relative" ref={ref}>
        <button onClick={() => setOpen(!open)} className="text-gray-400 hover:text-gray-600 text-sm">✎</button>
        {open && (
          <div className="absolute right-0 top-6 w-32 bg-white border border-gray-200 rounded-md shadow-md z-10">
            <button className="w-full text-left px-4 py-2 text-sm hover:bg-gray-100">Assign to</button>
        ...
        {selectedTask && (
          <div className="fixed top-0 right-0 w-full sm:w-96 h-full bg-white shadow-xl border-l p-4 z-50 overflow-y-auto">
            <div className="flex items-center justify-between border-b pb-2 mb-4">
              <h2 className="text-base font-semibold text-gray-700">Tasks-{selectedTask.id}</h2>
              <button onClick={() => setSelectedTask(null)} className="text-gray-500 hover:text-red-500 text-sm">
                ✕
              </button>
            </div>

            <div className="space-y-3 text-sm text-gray-700">
              {['description', 'type', 'status'].map((field) => (
                <TaskDetailField
                  key={field}
              ...
              <div>
                <h3 className="text-base font-semibold mt-4 mb-1">Description:</h3>
                <div className="flex justify-between items-start">
                  <p className="text-sm leading-snug text-gray-600">
                    Help arrange tables, book displays, banners, and decorations before and after the event.
                  </p>
                  <DropdownMenu />
                </div>
              </div>
              ...
              <div className="pt-4">
                <button className="px-4 py-1.5 bg-indigo-900 text-white text-sm rounded">Done</button>
              </div>
            </div>
          </div>
        )}
