...
import { updateEvent, toggleAcceptingVolunteers } from "@/app/(api)/events_api";
...
export default function EventDetailSidebar({
  ...
  const handleSave = async () => {
    try {
      await toggleAcceptingVolunteers(
        editableEvent.id,
        editableEvent.acceptingVolunteers
      );
  ...
  return (
    <div className="fixed top-0 right-0 w-full max-w-md h-full bg-white shadow-lg z-50 overflow-y-auto p-4 space-y-4">
      ...
      <div className="space-y-2">
        <label className="text-sm font-medium">Accepting Volunteers</label>
        {isEditing.acceptingVolunteers ? (
          <select
            className="w-full border border-gray-300 px-3 py-2 rounded"
            value={
              editableEvent.acceptingVolunteers !== undefined
                ? String(editableEvent.acceptingVolunteers)
                : ""
            }
            onChange={(e) =>
              handleChange(
          ...
          <div className="flex justify-between items-center bg-gray-100 px-3 py-2 rounded">
            <span>{editableEvent.acceptingVolunteers.toString()}</span>
            <button onClick={() => toggleEdit("acceptingVolunteers")}>
              <Pencil size={18} />
            </button>
          </div>
        )}
